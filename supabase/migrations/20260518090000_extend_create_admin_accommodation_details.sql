drop function if exists public.create_admin_accommodation(
  text,
  text,
  uuid,
  uuid,
...
begin
  ...
  values (
    ...
    coalesce(p_pool_type, 'none'::public.pool_detail_type),
    p_pool_system
  );
  ...
  loop
    ...
    values (v_accommodation_id, v_facility_id)
...
comment on function public.create_admin_accommodation(
  ...
  'Creates a published accommodation, details, price rules, facilities, contacts, pool, and pet policy in one transaction. Admin-only via explicit role check and table RLS.';
